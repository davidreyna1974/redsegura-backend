...
automatizada (112 tests, gatekeeper `mvn verify`) con una pasada de humo end-to-end sobre el
...
- **Tests de regresión añadidos** (`DeviceCertificationIT`): `CRUD-04b`
  ...
  (`replace_withNoManagementAddress_returns422`). Suite: **98 → 112 tests**.
...
- **Blast radius:** **local** (lógica interna de `asset-inventory-service`); no cambia el
  ...
  verde (112 tests, cobertura ≥ 70 %, lint).
